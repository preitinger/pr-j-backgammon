...
public class Bereit extends TemplateSearcher {
    private final Mat result = new Mat();

    public Bereit() throws IOException {
        super("bereit", ".png");
    }

    @Override
    protected Point run(Mat shot, Point lastPos, Point out, boolean trace) {
        System.out.println("Bereit.run: lastPos " + lastPos);
        if (lastPos != null && lastPos.y + template.height() <= shot.height()
            ...
            if (quickResult != null) {
                ...
                System.out.println("quickResult for Bereit: " + quickResult);
                return quickResult;
            }
        }
